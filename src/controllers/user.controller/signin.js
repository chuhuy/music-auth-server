...
const signin = (req, res) => {
    ...
    const signinSql = `SELECT user.secret FROM user WHERE user.username = '${req.body.username}'`;
        ...
        if(error) {
            ...
            bcrypt.compare(req.body.password, results[0]["secret"]).then((result) => {
                if(result) {
                    ...
                    const insertRefreshTokenSql = `UPDATE user SET user.refresh_token = '${refresh_token}' WHERE user.username = '${req.body.username}'`;
